...
export function createFormatBarWidgetConfig(framework: FrameworkProvider) {
  return {
  ...
  return {
    ...
    action: ctx => {
      ...
      try {
        ...
        const mode = docsService.list.getPrimaryMode(pageId);
        const ids = ctx.selectedModels.map(model => model.id);
